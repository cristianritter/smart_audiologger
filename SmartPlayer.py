    ...
    class MediaPlayer:
        ...
        def create_window(self):
            ...
            buttons_group = [[sg.In(key='CALENDAR', enable_events=True, visible=False), sg.CalendarButton('', image_filename=BUTTON_DICT['CALENDAR'], pad=(0,0),  
                            button_color=('white', self.default_bg_color), border_width=0, key='CALENDAR', format=('%Y%m%d'), tooltip="Abrir o CALENDARIO"),
                    self.button('START', BUTTON_DICT['START'], "Retornar ao inicio da gravação."),
                    self.button('REWIND', BUTTON_DICT['REWIND'], "Navegar entre os eventos marcados."),
                    self.button('PLAY', BUTTON_DICT['PLAY_OFF'], "Pausar ou Retormar a execução."),
                    self.button('FORWARD', BUTTON_DICT['FORWARD'], "Navegar entre os eventos marcados."),
                    ]]
